...
**v3 changes vs v2:** one added hard rule (rule 1a) forbidding the model from
deriving new figures by arithmetic on `facts` numbers — with a carve-out
letting it sum the top-N provider shares itself.

**v4 changes vs v3:** the v3 carve-out backfired. The model was asked to sum
the top-N provider shares and did the addition wrong (emitting 53.3% and
51.7% on the same day for a real 51.3% sum), so the validator rejected it and
the day fell back to template anyway. v4 removes the carve-out: no arithmetic
at all. `facts` now carries `rankings.provider_share_cumulative` — the
combined top-N shares precomputed in code — and rule 1a directs the model to
quote those verbatim instead of adding anything up.

**v5 changes vs v4:** wording only, no new rules. On 2026-09-10 the model
still combined two provider shares — but a *non-adjacent* pair (#1 + #3,
skipping #2), which no `provider_share_cumulative` entry covers, so it fell
back to template. v5 spells out that a combined-share claim is only allowed
when it names a *contiguous* run of the top providers matching a
`provider_share_cumulative` entry exactly — never an arbitrary subset — and
that when in doubt it should just state each provider's own share separately.
...
1a. Do not derive any figure by arithmetic. Quote percentages only as they
   literally appear in `facts` — never add, subtract, average, or otherwise
   combine numbers. This includes combined provider shares: if you want to
   say "the top three hold X% between them," take X *only* from a matching
   entry in `facts.rankings.provider_share_cumulative` (match its `top_n` to
   how many providers you name, and list them in the order they appear in
   that entry's `providers`). If there is no matching entry, don't make the
   claim. The providers you name must be a *contiguous* run from the top of
   the leaderboard (the top 2, top 3, top 4 …) — never an arbitrary subset
   and never skipping a provider. If you can't find a `provider_share_cumulative`
   entry that matches exactly, state each provider's own share separately
