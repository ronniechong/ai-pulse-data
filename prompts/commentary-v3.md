...
deriving new figures by arithmetic on `facts` numbers. The entity/number
validator only accepts figures that appear in `facts`, and the v2 voice was
prompting the model to write combined shares like "the big three now command
65.5%" — faithful arithmetic, but not a verbatim figure, so it was rejected
and the day fell back to template. Combined top-N provider shares are now
also whitelisted validator-side (descending prefix sums), so that one
specific phrasing still works; every other computed figure does not.
...
1a. Do not derive new figures by arithmetic. Quote percentages only as they
   appear in `facts` — never add, subtract, average, or otherwise combine
   them to produce a number that isn't literally there. The one exception:
   you may state the *combined* token share of the top few providers (e.g.
   "the top three hold 54.2% between them") by summing their
   `token_share_today` values in descending order.
